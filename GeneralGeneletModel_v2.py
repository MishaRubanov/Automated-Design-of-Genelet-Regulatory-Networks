...
def int_genelet_states(G_int_vec,G_tot,act_vec,dA_tot):
    ...
    for n in range(len(G_int_vec)):
        ...
        # where there is less activator than total genelet
            ...
            # print(dAin[act_vec[n]-1])
            # print(sum(G_tot[act_vec==act_vec[n]]))
            dAin[act_vec[n]-1]=sum(G_tot[act_vec==act_vec[n]])
            # print(dAin[act_vec[n]-1])
...
class GeneletNetwork:
    ...
    def simulate(self,t_vec,iteration,rnase='RnH',leak=0,rate_constants=[],rR=[],dA=[],rC=[],dB=[],rIr=[],rIc=[],dR=[]):
        ...
        if iteration == 1:
            ...
            # solving the ODEs
            self.sol = spi.solve_ivp(rates,[t[0],t[-1]],self.int_cond,method='LSODA',t_eval=t)
            ...
            # solving the odes
            sol_n = spi.solve_ivp(rates_n,[t_n[0],t_n[-1]],int_cond_n,t_eval=t_n,method='LSODA')
